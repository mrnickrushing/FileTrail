...
const FEATURES = [
  { icon: '📷', title: 'Scan anything in seconds', body: 'Point your camera and tap. FileTrail captures receipts, contracts, IDs, warranties, and letters — then reads every word with on-device OCR automatically.' },
  { icon: '🔍', title: 'Find it in under 3 seconds', body: 'Search by any word inside any document. "dentist invoice march" surfaces it instantly. No folders required — though we have those too.' },
  { icon: '🗂️', title: 'Auto-organises itself', body: 'FileTrail detects receipts, contracts, IDs, tax docs, and warranties automatically. Everything lands in the right place without you lifting a finger.' },
  { icon: '🔒', title: 'Fort Knox on your phone', body: 'Face ID or Touch ID locks your vault. Your documents never touch a server unless you choose cloud sync. Even then — encrypted end-to-end.' },
  { icon: '💾', title: 'Works 100% offline', body: 'No Wi-Fi? No problem. Capture, organise, search, and export even on a plane. Everything lives on your device first, cloud second.' },
  { icon: '📤', title: 'Share & export effortlessly', body: 'Send a single document or export your entire vault as a ZIP. Time-limited share links let you send docs to accountants without email attachments.' },
  { icon: '🤖', title: 'AI that actually helps', body: 'Pro AI reads your receipts and pulls out totals, dates, and vendors. Natural-language search means you can ask "find my car insurance from last year" and actually get it.' },
  { icon: '📧', title: 'Email-to-vault', body: 'Forward any email to your personal @filetrail.app address and the attachment lands straight in your vault. No more digging through Gmail for that invoice.' },
];
          ...
          <div className={styles.painGrid}>
            <div className={styles.painCard}>
              ...
              <ul className={styles.painList}>
                {["😤 Can't find that warranty when something breaks","📧 Hunting through emails for a PDF from 2 years ago","🗃️ Boxes of paper \"just in case\"","😰 Panic at tax time finding receipts","📁 A \"DOCS\" folder that's become a graveyard"].map(p => <li key={p}>{p}</li>)}
              </ul>
            </div>
            ...
            <div className={styles.solutionCard}>
              ...
              <ul className={styles.solutionList}>
                {["✅ Every document scanned and searchable in seconds","✅ OCR reads the text so you search by content, not filename","✅ Auto-detects warranties, receipts, IDs — files them correctly","✅ Tax time = ZIP export, done","✅ One app. Everything. Always on your phone."].map(s => <li key={s}>{s}</li>)}
              </ul>
            </div>
          </div>
        </div>
